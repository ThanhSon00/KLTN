...
export default function UserNoAnswer({ user }: Props) {
    const [questions, setQuestions] = useState<Question[]>([]);
    const [page, setPage] = useState<number>(1);
    const [hasMore, setHasMore] = useState<boolean>(false);
    const loadQuestions = async (userId: string) => {
      const result = await getUserQuestions(userId, { amount: 5, page, answersCount: 0 });
        ...
        <section className="loop-section" id="section-questions">
            ...
            ? <div className="card">
              <div className="card-body">
                <div className="no-results not-found text-center">
                  ...
                  <p className="mb-2rem">
                    Có vẻ như bạn không có câu hỏi nào chưa trả lời
                  </p>
                </div>
              </div>
            </div> 
          ...
          <div className="main-pagination">
            <div className="pagination">
                ...
                {questions.length > 0 && <span aria-current="page" className="page-numbers current">{page}</span>}
                {hasMore && 
                    <>
                        <a className="page-numbers" href="/" onClick={e => handlePageClick(e, page + 1)}>{page + 1}</a>
